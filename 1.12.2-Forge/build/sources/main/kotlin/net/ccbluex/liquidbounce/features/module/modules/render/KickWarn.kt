...
@ModuleInfo(name = "KickWarn", Chinese = "踢出警告", description = "Skid", category = ModuleCategory.RENDER)
class KickWarn : Module() {
    ...
    private var translat = Translate2(0f, 0f)
    ...
    fun onRender2D(e: Render2DEvent) {
        ...
        // TooMany Packets
        if (packetsCheck.get()) {
            Fonts.com40.drawString(
                "TooMany Packets",
                x + (100f - Fonts.com40.getStringWidth("TooMany Packets")) / 2f,
                y.toFloat(),
                Color(255, 255, 255, 200).rgb,
                false
            )
            RenderUtils.drawRect(x.toFloat(), y + 10f, width.toFloat(), y + 25f, Color(0, 0, 0, 125))
            RenderUtils.drawRect(
                x.toFloat(),
                y + 10f,
                Math.min(x + translat.x, width.toFloat()),
                y + 25f,
        ...
        if (vanillaCheck.get()) {
            Fonts.com40.drawString(
                "Vanilla Kick",
                x + (100f - Fonts.com40.getStringWidth("Vanilla Kick")) / 2f,
                y + 35f,
                Color(255, 255, 255, 200).rgb,
                false
            )
            RenderUtils.drawRect(x.toFloat(), y + 45f, width.toFloat(), y + 60f, Color(0, 0, 0, 125))
            RenderUtils.drawRect(
                x.toFloat(),
                y + 45f,
                Math.min(x + translat.y, width.toFloat()),
                y + 60f,
