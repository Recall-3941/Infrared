...
import me.utils.player.MoveUtil
...
@ModuleInfo(name = "KillAura", description = "Automatically attacks targets around you.", category = ModuleCategory.COMBAT, keyBind = Keyboard.KEY_R, Chinese = "杀戮光环")
class KillAura : Module() {
    ...
    // Modes
    val rotations = ListValue("RotationMode", arrayOf("Vanilla", "Other", "BackTrack", "Hyt"), "Hyt")
    ...
    private fun updateRotations(entity: IEntity): Boolean {
        ...
        if (rotations.get().equals("Hyt", ignoreCase = true)) {
            var boundingBox = entity.entityBoundingBox
            if (maxTurnSpeed.get() <= 0F)
                return true
    ...
    private val cancelRun: Boolean
        inline get() = mc.thePlayer!!.spectator || !isAlive(mc.thePlayer!!)
                ...
                || noScaffoldValue.get() && LiquidBounce.moduleManager.getModule(Scaffold::class.java).state
