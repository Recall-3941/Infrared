...
class InvManager : Module() {
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (!InventoryUtils.CLICK_TIMER.hasTimePassed(delay) || (combatManager.inCombat && noCombatValue.get()) || (!classProvider.isGuiInventory(mc.currentScreen) && invOpenValue.get()) || noMoveValue.get() && isMoving || thePlayer.openContainer != null && thePlayer.openContainer!!.windowId != 0) return
    ...
    private fun move(item: Int, isArmorSlot: Boolean): Boolean {
        if (!isArmorSlot && item < 9 && !classProvider.isGuiInventory(mc.currentScreen)) {
            ...
            mc.netHandler.addToSendQueue(
                createUseItemPacket(
                    mc.thePlayer?.heldItem,WEnumHand.MAIN_HAND
                )
            )
            mc.netHandler.addToSendQueue(
