 ...
 * FDPClient
 */
class KeySelectUI(val info: KeyInfo) : PopUI("Select") {
    private var str = ""
    private var modules = LiquidBounce.moduleManager.modules.toList()
    private val singleHeight = 4F + Fonts.font35.fontHeight
    private var stroll = 0
    private var maxStroll = modules.size * singleHeight
    private val height = 8F + Fonts.font40.fontHeight + Fonts.font35.fontHeight + 0.5F

    override fun render() {
        // modules
        var yOffset = height - stroll + 5F
        if (str.startsWith(".")) {
            Fonts.font35.drawString("Add", 8F, singleHeight + yOffset, Color.BLACK.rgb, false)
        } else {
            for (module in modules) {
                ...
                yOffset = yOffset+ singleHeight
            }
        }
        RenderUtils.drawRect(0F, 8F + Fonts.font40.fontHeight, baseWidth.toFloat(), height + 5F, Color.WHITE.rgb)
        RenderUtils.drawRect(0F, baseHeight - singleHeight, baseWidth.toFloat(), baseHeight.toFloat(), Color.WHITE.rgb)

        // search bar
        Fonts.font35.drawString(str.ifEmpty { "Search" }, 8F, 8F + Fonts.font40.fontHeight + 4F, Color.LIGHT_GRAY.rgb, false)
