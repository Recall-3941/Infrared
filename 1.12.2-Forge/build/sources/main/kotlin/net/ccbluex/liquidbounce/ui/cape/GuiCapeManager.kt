...
object GuiCapeManager : GuiScreen() {
    ...
    init {
        arrayOf("bilibili","bilibilitv", "huoying", "jiaran", "liquidbounce","lunar","paimon","rq","skyrim","stars").forEach {
            try {
                embeddedCapes.add(loadCapeFromResource(it, "assets/minecraft/loserline/cape/$it.png"))
            } catch (e: Throwable){
    ...
    override fun initGui() {
        this.buttonList.add(GuiButton(0, 0, 0, net.ccbluex.liquidbounce.ui.font.Fonts.minecraftFont.getStringWidth("< Back") + 10, 20, "< Back"))
        this.buttonList.add(GuiButton(1, (width * 0.3).toInt(), (height * 0.5).toInt(), net.ccbluex.liquidbounce.ui.font.Fonts.minecraftFont.getStringWidth("<-") + 10, 20, "<-"))
        this.buttonList.add(GuiButton(2, (width * 0.7).toInt(), (height * 0.5).toInt(), net.ccbluex.liquidbounce.ui.font.Fonts.minecraftFont.getStringWidth("->") + 10, 20, "->"))
    }
    ...
    override fun drawScreen(p_drawScreen_1_: Int, p_drawScreen_2_: Int, p_drawScreen_3_: Float) {
        ...
        GL11.glPushMatrix()
        net.ccbluex.liquidbounce.ui.font.Fonts.minecraftFont.drawCenteredString(if (nowCape == null) { "§cNONE" } else { "§a${nowCape!!.name}" }, width * 0.50f, height * 0.23f, -1, false)
        GL11.glScalef(2f, 2f, 2f)
        net.ccbluex.liquidbounce.ui.font.Fonts.minecraftFont.drawCenteredString("Cape Manager", width * 0.25f, height * 0.03f, -1, false)
        GL11.glPopMatrix()
