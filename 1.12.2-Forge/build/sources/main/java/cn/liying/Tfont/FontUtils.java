...
public class FontUtils
{
    ...
    public FontUtils(final Font font, final int fontType, final int size, final boolean allChar) {
        this(font, fontType, size, 0, allChar);
    }
    
    public FontUtils(final Font font, final int fontType, final int size, final int kerning, final boolean allChar) {
        this(font, fontType, size, kerning, allChar, 0);
    }
    
    public FontUtils(final Font font, final int fontType, final int size, final int kerning, final boolean allChar, final int yAddon) {
    ...
    private Font getFont(final String fontName, final int fontType, final int size) {
        Font font = null;
        try {
            final InputStream ex = FontManager.class.getResourceAsStream("/assets/minecraft/loserline/fonts/" + fontName);
    ...
    public void drawLimitedStringWithAlpha(String text, float x, float y, final int color, final float alpha, final float maxWidth) {
        ...
        for (final char c : characters) {
            ...
            Label_0367: {
                if (c != '1' && (index == 0 || index == characters.length - 1 || characters[index - 1] != '1')) {
                    if (index >= 1 && characters[index - 1] == '1') {
                        break Label_0367;
                    }
                    GL11.glPushMatrix();
                    this.unicodeFont.drawString(Character.toString(c), (double)x, (double)y, VisualUtils.reAlpha(currentColor, alpha), false);
                    GL11.glPopMatrix();
                ...
                else if (c == '1' && index != characters.length - 1) {
    ...
    public float getHeight() {
        return this.unicodeFont.getHeight("FluxClientIsThatBestClarinet.") / 2.0f;
    }
