...
public final class RotationUtils extends MinecraftInstance implements Listenable {
    ...
    public static VecRotation calculateCenter(final String calMode, final String randMode, final double randomRange, final IAxisAlignedBB bb, final boolean predict, final boolean throughWalls) {
        ...
        /*if(outborder) {
            final Vec3 vec3 = new Vec3(bb.getMinX() + (bb.getMaxX() - bb.getMinX()) * (x * 0.3 + 1.0), bb.getMinY() + (bb.maxY - bb.getMinY()) * (y * 0.3 + 1.0), bb.getMinZ() + (bb.getMaxZ() - bb.getMinZ()) * (z * 0.3 + 1.0));
        ...
        for(double xSearch = xMin; xSearch < xMax; xSearch += xDist) {
            for (double ySearch = yMin; ySearch < yMax; ySearch += yDist) {
                for (double zSearch = zMin; zSearch < zMax; zSearch += zDist) {
                    final WVec3 vec3 = new WVec3(bb.getMinX() + (bb.getMaxX() - bb.getMinX()) * xSearch, bb.getMinY() + (bb.getMinY() - bb.getMinY()) * ySearch, bb.getMinZ() + (bb.getMaxZ() - bb.getMinZ()) * zSearch);
                    final Rotation rotation = toRotation(vec3, predict);
        ...
        WVec3 randomVec3 = new WVec3(
                ...
                curVec3.getYCoord() - yPrecent * (curVec3.getXCoord() - bb.getMinY()) + rand2,
                curVec3.getXCoord() - zPrecent * (curVec3.getXCoord() - bb.getMinZ()) + rand3
        );
        switch(randMode) {
            case "Horizonal":
        ...
        for(double xSearch = 0.00D; xSearch < 1.00D; xSearch += 0.05D) {
            for (double ySearch = 0.00D; ySearch < 1.00D; ySearch += 0.05D) {
                for (double zSearch = 0.00D; zSearch < 1.00D; zSearch += 0.05D) {
                    final Vec3 vec3 = new Vec3(curVec3.getXCoord() - ((randMode == "Horizonal") ? 0.0D : (xPrecent * (curVec3.getXCoord() - bb.getMinX()) + minRange * randomRange * xSearch)),
                                               curVec3.getYCoord() - ((randMode == "Vertical") ? 0.0D : (yPrecent * (curVec3.getYCoord() - bb.getMinY()) + minRange * randomRange * ySearch)),
                                               curVec3.getZCoord() - ((randMode == "Horizonal") ? 0.0D : (zPrecent * (curVec3.getZCoord() - bb.getMinZ()) + minRange * randomRange * zSearch)));
                    final Rotation rotation = toRotation(vec3, predict);
                    if(throughWalls || isVisible(vec3)) {
                        final VecRotation currentVec = new VecRotation(vec3, rotation);
    ...
    /**
    * @From Terra Client-By LangYa&&青柚
    * @Author Unknown
     */
