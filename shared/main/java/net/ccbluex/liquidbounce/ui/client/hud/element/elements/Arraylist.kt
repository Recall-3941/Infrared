...
class Arraylist(x: Double = 1.0, y: Double = 2.0, scale: Float = 1F,
    ...
    private val colorModeValue = ListValue("Text-Color", arrayOf("Custom", "Random", "Rainbow","OtherRainbow","ALLRainbow","Bainbow","TwoRainbow","OriginalRainbow","LRainbow","NovoRainbow"), "Custom")
    ...
    private val rectColorModeValue = ListValue("Rect-Color", arrayOf("Custom", "Random", "Rainbow","OtherRainbow","ALLRainbow","Bainbow","TwoRainbow","OriginalRainbow","LRainbow","NovoRainbow"), "Rainbow")
    ...
    private val backgroundColorModeValue = ListValue("Background-Color", arrayOf("Custom", "Random", "Rainbow","OtherRainbow","ALLRainbow","Bainbow","TwoRainbow","OriginalRainbow","LRainbow","NovoRainbow"), "Custom")
        ...
        when (side.horizontal) {
            Horizontal.RIGHT, Horizontal.MIDDLE -> {
                modules.forEachIndexed { index, module ->
                    ...
                    RainbowShader.begin(backgroundRectRainbow, if (rainbowX.get() == 0.0F) 0.0F else 1.0F / rainbowX.get(), if (rainbowY.get() == 0.0F) 0.0F else 1.0F / rainbowY.get(), System.currentTimeMillis() % 10000 / 10000F).use {
                        RenderUtils.drawRect(
                            ...
                            backgroundColorMode.equals("LRainbow", ignoreCase = true) -> Colors.getRainbow(-2000, (yPos * 8.toFloat()).toInt())
                                backgroundColorMode.equals("NovoRainbow", ignoreCase = true) -> ColorUtils.GodLight(index).rgb
                    ...
                    val hud = LiquidBounce.moduleManager.getModule(GodLightSync::class.java) as GodLightSync
                    val color3 = VisualUtils.getGradientOffset(Color(hud.r.get(), hud.g.get(), hud.b.get(),1), Color(hud.r2.get(), hud.g2.get(), hud.b2.get(),1), (Math.abs(
                        System.currentTimeMillis() / 814
                    ...
                    RainbowFontShader.begin(rainbow, if (rainbowX.get() == 0.0F) 0.0F else 1.0F / rainbowX.get(), if (rainbowY.get() == 0.0F) 0.0F else 1.0F / rainbowY.get(), System.currentTimeMillis() % 10000 / 10000F).use {
                        fontRenderer.drawString(displayString, xPos - if (rectMode.equals("right", true)) 3 else 0, module.higt + textY, when {
                            ...
                            colorMode.equals("NovoRainbow", ignoreCase = true) -> color3
                            else -> customColor
                        }, textShadow)
                    }
                    ...
                    if (!rectMode.equals("none", true)) {
                        ...
                        RainbowShader.begin(rectRainbow, if (rainbowX.get() == 0.0F) 0.0F else 1.0F / rainbowX.get(), if (rainbowY.get() == 0.0F) 0.0F else 1.0F / rainbowY.get(), System.currentTimeMillis() % 10000 / 10000F).use {
                            val rectColor = when {
                                ...
                                rectColorMode.equals("NovoRainbow", ignoreCase = true) -> ColorUtils.GodLight(index).rgb
