...
//        GL11.glPushMatrix()
        ...
        Fonts.font40.drawString("KeyBindManager", width * 0.21f * 0.5f, height * 0.2f * 0.5f, Color.WHITE.rgb, false)
//        GL11.glPopMatrix()
        GL11.glScalef(0.5f, 0.5f, 0.5f)

        GL11.glTranslatef(width * 0.2f, height * 0.2f + Fonts.font40.fontHeight * 2.3f, 0F)
        val scale = mcWidth / baseWidth.toFloat()
        // 用scale可以方便些
        GL11.glScalef(scale, scale, scale)

        RenderUtils.drawRoundedRect(0F, 0F, baseWidth.toFloat(), baseHeight.toFloat(),14F, Color.WHITE.rgb)
        ...
        if (Mouse.hasWheel()) {
            ...
            if (wheel != 0) { // 没滚轮，只是正常鼠标移动
                if (popUI != null) {
                    ...
                    val scaledMouseY = (mouseY - (height * 0.2f + Fonts.font40.fontHeight * 2.3f)) / scale
