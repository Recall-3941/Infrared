...
class Keystrokes : Element(5.0, 25.0, 1.5F, Side.default()) {
    ...
    public var floatX: Float? = null
    public var floatY: Float? = null
    init {
    ...
    override fun drawElement(): Border? {
        ...
        if(keyStyleValue.get().equals("Custom")) {
            for (keyStroke in keys) {
                keyStroke.renderCustom(animSpeedValue.get(), backGroundColor, textColor, highLightPercent.get(), outline.get(), outlineBoldValue.get(), fontValue.get(),1f, this.renderX.toFloat(), this.renderY.toFloat(), scale)
            }}
        ...
        if(keyStyleValue.get().equals("Jello")) {
            ...
            RenderUtils.drawImage(classProvider.createResourceLocation("loserline/misc/keystrokes.png"),
                (-3.5f).toInt(),
                (-3.5f).toInt(), 54, 54)
        ...
        if(keyStyleValue.get().equals("Juul")) {
            for (keyStroke in juulkeys) {
                keyStroke.renderJuul(animSpeedValue.get(), backGroundColor, textColor, highLightPercent.get(), outline.get(), outlineBoldValue.get(), fontValue.get(), 1f, this.renderX.toFloat(), this.renderY.toFloat(), scale)
            }
            ...
            Fonts.font30.drawString(juulLeft, 15.5f - (fontRenderer.getStringWidth(juulLeft) / 2f) + 1f, 39.5f - (fontRenderer.fontHeight / 2f) + 2f, textColor.rgb)
            Fonts.font30.drawString(juulRight, 39.5f - (fontRenderer.getStringWidth(juulRight).toFloat() / 2f) + 1f, 39.5f - (fontRenderer.fontHeight.toFloat() / 2f) + 2f, textColor.rgb)
        }
...
class KeyStroke(val key: IKeyBinding, val posX: Int, val posY: Int, val width: Int, val height: Int) {
    ...
    fun renderCustom(speed: Int, bgColor: Color, textColor: Color, highLightPct: Float, outline: Boolean, outlineBold: Int, font: IFontRenderer, blurRadius: Float, renderX: Float, renderY: Float, scale: Float) {
        GL11.glPushMatrix()
        GL11.glTranslatef(posX.toFloat(), posY.toFloat(), 0F)

        if (blurRadius != 0f) {
            //BlurBuffer.blurArea((renderX + posX) * scale, (renderY + posY) * scale, width * scale, height * scale)
        }

        RenderUtils.drawShadowWithCustomAlpha(0F, 0F, width.toFloat(), height.toFloat(), 240f)
        ...
        font.drawString(keyName, centerX - (font.getStringWidth(keyName) / 2) + 1, centerY - (font.fontHeight / 2) + 1, textColor.rgb)
        if (outline) {
    ...
    fun renderJuul(speed: Int, bgColor: Color, textColor: Color, highLightPct: Float, outline: Boolean, outlineBold: Int, font: IFontRenderer, blurRadius: Float, renderX: Float, renderY: Float, scale: Float) {
        GL11.glPushMatrix()
        GL11.glTranslatef(posX.toFloat(), posY.toFloat(), 0F)
        val nowTime = System.currentTimeMillis()
        ...
        Fonts.font30.drawString(keyName, width / 2 - (font.getStringWidth(keyName) / 2) + 1, height / 2 - (font.fontHeight / 2) + 2, textColor.rgb)
        GL11.glPopMatrix()
    }
...
//            GL11.glPushMatrix()
        // BlurBuffer.blurArea(floatX.toFloat(), (posY + 11).toFloat(), width.toFloat(), height.toFloat() )
//            GL11.glPopMatrix()
        //GL11.glTranslated(renderX.toDouble(), renderY.toDouble(), 0.0)
        //BlurBuffer.blurRoundArea((renderX ) , (renderY ) , width.toFloat(), height.toFloat(),10)
        //GL11.glPopMatrix()
    }
