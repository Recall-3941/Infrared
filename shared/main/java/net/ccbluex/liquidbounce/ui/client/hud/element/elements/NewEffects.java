...
public class NewEffects extends Element {
    ...
    public Border drawElement() {
        ...
        for (final IPotionEffect potionEffect : mc.getThePlayer().getActivePotionEffects()) {
            ...
            RenderUtils.drawRectPotion(0, potionData.translate.getY(), 120, potionData.translate.getY() + 30F, RenderUtils.reAlpha(HanaBiColors.GREY.c, 0.1F));
            RenderUtils.drawRectPotion(0, potionData.translate.getY(), potionData.animationX, potionData.translate.getY() + 30F, RenderUtils.reAlpha((new Color(34, 24, 20)).brighter().getRGB(), 0.3F));
            RenderUtils.drawShadow(0, Math.round(potionData.translate.getY()), 120, 30);
            float posY = potionData.translate.getY() + 13F;
            Fonts.com35.drawString(name + " " + intToRomanByGreedy(potionEffect.getAmplifier() + 1), 29F, posY - mc.getFontRendererObj().getFontHeight(), RenderUtils.reAlpha(HanaBiColors.WHITE.c, 0.8F));
            Fonts.font35.drawString(potionEffect.getDurationString(), 29F, posY + 4.0F, RenderUtils.reAlpha((new Color(200, 200, 200)).getRGB(), 0.5F));
