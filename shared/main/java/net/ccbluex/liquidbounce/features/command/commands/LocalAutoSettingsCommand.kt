                ...
                args[1].equals("load", ignoreCase = true) -> {
                    if (args.size > 2) {
                        ...
                        if (scriptFile.exists()) {
                            try {
                                LiquidBounce.isUsingCloudCfg = false
                                chat("§9Loading settings...")
                                val settings = scriptFile.readText()
                                chat("§9Set settings...")
                                SettingsUtils.executeScript(settings)
                                chat("§6Settings applied successfully.")
                                LiquidBounce.hud.addNotification(Notification("Configs","Updated Settings", NotifyType.INFO))
                                playEdit()
                            } catch (e: IOException) {
                                e.printStackTrace()
                            }
                ...
                args[1].equals("save", ignoreCase = true) -> {
                    if (args.size > 2) {
                        ...
                        try {
                            ...
                            val settingsScript = SettingsUtils.generateScript(true, true, true)
